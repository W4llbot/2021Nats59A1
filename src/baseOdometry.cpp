...
#define USING_LATERAL_ENCD false;
...
void baseOdometry(void * ignore){
  ...
  while(!COMPETITION_MODE || competition::is_autonomous()){
    if(inertial.is_calibrating()) {
      ...
      position.angle = inertial.get_rotation()*toRad;
      ...
      master.print(2, 0, "%.2f", (BL.get_position() - BR.get_position())/baseWidth*toDeg);
...
void waitCallibrate() {
  Imu inertial(inertialPort);
  while(inertial.is_calibrating()) delay(5);
}
